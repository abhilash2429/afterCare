...
def verify_circle_token(token):
    try:
        ...
        raise Unauthorized("invalid circle token: %s" % type(exc).__name__)
    if claims.get("role") != "caregiver" or not claims.get("circleId"):
        raise Unauthorized("invalid circle token claims")
    return claims
...
def _cognito_claims(token, header):
    ...
    try:
        ...
        raise Unauthorized("invalid id token: %s" % type(exc).__name__)
...
def principal_from_event(event):
    ...
    raise Unauthorized("unsupported token")
